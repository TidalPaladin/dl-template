...
class FakeModel(HydraMixin, pl.LightningModule):
    def __init__(self, in_features: int, out_features: int, num_classes: int = 10, kernel: int = 3):
        ...
        self.train_metrics = MetricCollection({
            ...
            "F1": F1(compute_on_step=True, dist_sync_on_step=False),
        })
        ...
        self.test_metrics = MetricCollection({
            ...
            "F1": F1(compute_on_step=False, dist_sync_on_step=True),
        })
    ...
    def _base_step(self, mode: str, batch: Any, batch_nb: int) -> Tensor:
        ...
        with torch.no_grad():
            ...
            self.log(f"{prefix}/loss", loss, on_step=self.training, on_epoch=not self.training)
    ...
    def training_step(self, batch, batch_nb):
        ...
        return self._base_step('train', batch, batcn_nb)

    def validation_step(self, batch, batch_nb):
        return self._base_step('val', batch, batcn_nb)

    def test_step(self, batch, batch_nb):
        return self._base_step('test', batch, batcn_nb)
