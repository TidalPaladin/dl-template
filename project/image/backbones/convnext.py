...
from flash.core.utilities.imports import _TORCHVISION_AVAILABLE, requires
from flash.core.utilities.providers import _TORCHVISION
from flash.core.utilities.url_error import catch_url_error
from torch import Tensor

from ...core.adapter import BackboneAdapter, SequentialList
...
class ConvNextAdapter(BackboneAdapter[List[Tensor]]):
    ...
    def extract_body(cls, model: nn.Module) -> nn.Module:
        features = cast(List[nn.Module], model.features)
        layers = nn.ModuleList([l for i, l in enumerate(features[1:]) if i % 2 == 0])
        downsample = nn.ModuleList([l for i, l in enumerate(features[1:]) if i % 2 == 1])
        assert len(layers) == len(downsample) + 1
        blocks = SequentialList(*[nn.Sequential(l, ds) for l, ds in zip(layers, downsample)])
